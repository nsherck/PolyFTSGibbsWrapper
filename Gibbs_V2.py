...
class Gibbs_System():
	...
	def SetSpeciesCTotal(self,_SpeciesCTotal):
		...
		if self.Nspecies != len(_SpeciesCTotal):
			print('WARNING: The number of species does not match the length of the list of number densities!')
	...
	def SCFTModel(self,rhoA,rhoB,N_A,N_B,Int,a_list,IncludeRPA):
		...
		# NOTE: RPA inside the spinodal is unstable to fluctuations
			if N_B > 1:
				print("WARNING: RPA model only treats species B as a solvent, i.e. N_B == 1!")
			_kmin = 0.
	...
	def WritePolyFTSInput(self,dummyvarnames,variables,modelpath):
		...
		with open('template.in','r') as myfile:
			...
			if self.Iteration > 1:
				...
				ini=re.sub('__NUMBLOCKS__','2000',ini)
			else:
				ini=re.sub('__READFIELDS__','No',ini)
				ini=re.sub('__NUMBLOCKS__','2000',ini)
	...
	def WriteStats(self):
		...
		if self.Iteration == 1: # initialize file
			...
			header_temp += "HI  HII  PI  PII  "
			for i in range(self.NSpecies):
				header_temp += 'muI_{}  muII_{}  '.format(i+1,i+1)
			header_temp +="\n"
	...
	def UpdateParameters(self,Operator_List):
		...
		for i in range(nparam):
			...
			temp_Operators.extend([Operator_List[0][i][0],Operator_List[1][i][0]])
		...
		if self.JobType == 'CL':
			...
			print("Max relative error: {0:3.3e}".format(max_relative_error))
			if max_relative_error > self.OperatorRelTol: # slow down
				...
				print('Slowing down: blocks {0} scale {1:3.3e}'.format(self.NPolyFTSBlocks,scale))
				...
				print('Speeding up: {}'.format(self.NPolyFTSBlocks))
			else:
				pass
		...
		#if self.NPolyFTSBlocks > self.NPolyFTSBlocksmin:
		#	self.NPolyFTSBlocks = int(self.NPolyFTSBlocksmin)
		#	print("Reseting NPolyFTSBlocks from {} to {}.".format(self.NPolyFTSBlocks,self.NPolyFTSBlocksmin))
		...
		for indx, var in enumerate(self.ValuesCurrent[::2]):
			if indx == 0: # volume fraction 
				...
				if self.ValuesCurrent[indx*2] < self.VolFracBounds[0]:
					self.ValuesCurrent[indx*2] = 0.35
				if self.ValuesCurrent[indx*2] > self.VolFracBounds[1]:
					self.ValuesCurrent[indx*2] = 0.65
				...
				#	self.ValuesCurrent[indx*2] = 0.+1e-6
					...
					#dteff = self.Dt[indx]
					#cnt = 0.
					print('Value for operator {} < 0; iterating...'.format(indx))
					#while temp_val < 0.:
					#	dteff = dteff/2.
				...
				if self.ValuesCurrent[indx*2]  > self.SpeciesCTotal[indx-1]/self.ValuesCurrent[0]:
					print('Value for operator {} > [max]; setting to [max]...'.format(indx))
