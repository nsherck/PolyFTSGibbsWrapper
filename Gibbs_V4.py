...
class Gibbs_System():
    ...
    def __init__(self,_Program,_Nspecies=1):
        ''' 
            ...
            Assumption:
                All neutral species are defined after charged species
        ...
        self.Charges           = np.zeros(_Nspecies) # charges of all species, 
        ...
        self.ChargePairs      = [] # pairs of indices of species that make up effective neutral species
    ...
    def SetChargedSpecies(self):
        ...
        self.NchargedSpecies = np.len(self.ChargedSpecies)
        
    def SetCharges(self,_Charges):
        ''' Set charges of all species in the system '''
        #check: len(_Charges) = self.Nspecies
    ...
    def SetChargedPairs(self,_ChargedPairs):
        ...
        self.NeffSpecies = len(self.ChargedPairs) + (self.NSpecies - self.ChargedSpecies)
    ...
    def GetEffC(self, Pair, Box):
        ''' Get the concentration of a charged pair
            Box: 1 for box 1, 2 for box 2 '''
        i1,i2 = Pair
        z1, z2 = [self.Valencies[i1],self.Valencies[i2]]
        C = z2 * self.ValuesCurrent[2*(i1+1)+(Box-1)] + z1 * self.ValuesCurrent[2*(i2+1)+(Box-1)]
        return C
    ...
    def GetChargedC(self, i):
        ...
        C = 0
        for indx, Pair in enumerate(self.ChargedPair):
            if i in Pair:
                j = Pair[np.where(np.array(Pair) != i)[0]]
    ...
    def GetChargedDMu(self,i):
        ...
        for indx, Pair in enumerate(self.ChargedPair):
            if i in Pair:
                j = Pair[np.where(np.array(Pair) != i)[0]]
                zj = self.Valencies[j] # valency of the j species in pair
                dMu_pair = self.DvalsCurrent[indx+2]
                dMu += zj * dMu_pair 
        return dMu
    ...
    def GetDtBounds(self, i, Ci_high = None, Ci_low = None):
        ...
        CTotal = self.CTotal[i]
        dMui = self.DvalsCurrent[i+1]        
        ...
        if Ci_low == None or Ci_low < 0:
            Ci_low = 0.       
        Dt_low = (Ci1 - Ci_high/self.ValuesCurrent[0]) / dMui
        Dt_high = (Ci1 - Ci_low/self.ValuesCurrent[0]) / dMui
    ...
    def UpdateChargedDtC(self):
        ...
        for i in range(self.Nspecies):
            if i <= len(self.ChargedPairs):
                Dt_low, Dt_high = self.GetDtBounds(i)
                Dt_bound_charged.extend([Dt_low, Dt_high])
            else:               
                Dt_low, Dt_high = self.GetDtBounds(i+1)
                Dt[i+1] = min(Dt_low, Dt_high, DtCMax)            

        Dt_charged = min(Dt_bound_charged)
        Dt[1:len(self.ChargedPairs)+1] = Dt_charged
        self.SetDt(Dt)
    ...
    def WriteStats(self):
        ...
        if self.Iteration == 1: # initialize file
            ...
            for i in range(self.NeutralSpecies):
    ...
    def GetOperatorStats(self,RunPath_):
        ...
        if self.NchargedSpecies > 0:                
            Mu_List = operator_statistics[2:]
            operator_statistics_0 = operator_statistics
            operator_statistics = operator_statistics_0[:2]
            EffMu_List = []            
            # calculate effective chemical potentials of charged pairs
            for [i1,i2] in self.ChargedPairs:
                Mu1 = Mu_List[i1][0]
                Mu2 = Mu_List[i2][0]
                Mu1Err = Mu_List[i1][1]
                Mu2Err = Mu_List[i2][1]
                Mu, MuErr = self.GetEffMu([i1,i2],Mu1,Mu2,Mu1Err,Mu2Err)
                AvgNsamples = np.mean(Mu_List[i1][2],Mu_List[i2][0][2])
                EffMu_List.append([Mu, MuErr,AvgNsamples])
            # adding chemical potential of neutral species to list
            for i in self.NeutralSpecies:
                EffMu_List.append(Mu_List[i])
            operator_statistics.extend(EffMu_List)            
    ...
    def UpdateParameters(self,Operator_List):
        ''' Update the parameters based on differences in Pressures and Chemical potentials between 
            ...
            program == polyFTS:
                dUCurrent == [dH, dP, dMu_1, ...., dMu_Nspecies] if neutral
                dUCurrent == [dH, dP, dMuPair_1, ...., dMuPair_NchargedSpecies-1, dMuNeutral_1, dMuNeutral_NneutralSpecies] if charged
        ...
        if self.ReRun != True or self.UseReRun != True: # only update if not rerunning
            ...
            if self.NchargedSpecies == 0:
                ...
                if self.Barostat != True:
                    for indx in range(self.NeffSpecies + 1):                     
                        var = self.ValuesCurrent[indx*2]
                        if indx == 0: # volume fraction
                            if self.Ensemble == 'NVT':
                                self.ValuesCurrent[indx*2] = var + self.Dt[indx]*self.DvalsCurrent[indx+1]
                                if self.ValuesCurrent[indx*2] < self.VolFracBounds[0]:
                    ...
                    #update species                               
                            ...
                            # update concentration of charged pairs in box 1:
                            Cpair1_List = []
                            if indx-1 < len(self.ChargePairs):
                                Pair = self.ChargedPair[indx-1]                            
                                Cpair1 = self.GetEffC(Pair,1)
                                Cpair1 = Cpair1 - self.Dt[indx] * self.DvalsCurrent[indx+1] 
                                Cpair1_List.append(Cpair1)
                            else: # update neutral species
                                indx_tmp = indx + 1 # index of the neutral species in Values Current
                                var = self.ValuesCurrent[indx_tmp*2]
                                self.ValuesCurrent[indx_tmp*2] = var - self.Dt[indx] * self.DvalsCurrent[indx+1] 
                    self.Cpair1 = Cpair1_List 
                    ...
                    #update phase II for species i
                    for i in range(len(self.NSpecies)):
                        Ci1 = self.ValuesCurrent[(i+1)*2]
